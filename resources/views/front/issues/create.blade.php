...
<body style="background:white;overflow-x:hidden">
    ...
    <div class="container mb-5" style="position: absolute;left:0;right:0">
        @if (session('message'))
            <div class="alert-success mt-3 px-2 py-2 mb-2 text-center" id="alertID">
                {{ session('message') }}
            </div>
        @endif
            ...
            <form class="w-100 justify-content-center" method="post" action="{{ url('storeIssue') }}">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <select id="inputState" name="type" class="form-control">
                            ...
                            @foreach ($bugTypes as $bug)
                                <option value="{{ $bug->name }}">{{ $bug->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group col-md-12">
                        ...
                        <textarea required class="form-control" name="description" id="issue" cols="30"
                            rows="5"></textarea>
                    </div>
                </div>
